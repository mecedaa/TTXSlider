...
// Bump this whenever the questions change. On start-up, saved state carrying an
// older stamp has its questions replaced and its responses cleared, because
// answers recorded against different questions and scales cannot be compared.
const QUESTION_VERSION = 3;

const DEFAULT_STATE = {
  session: {
    qversion: QUESTION_VERSION,
    ...
    questions: [
      { text: 'How concerned are you at this point?',
        left: 'Not at all', right: 'Panic' },
      { text: 'What is your estimate of the economic impact at this point?',
        left: '$0', right: '$10bn' },
      { text: 'How well do you understand the cause of the problem at this point?',
        left: 'Not very well', right: 'Complete understanding' }
    ]
  },
...
function migrate() {
  if (!state.session) state.session = JSON.parse(JSON.stringify(DEFAULT_STATE.session));
  if (state.session.qversion === QUESTION_VERSION) return false;
  state.session.qversion = QUESTION_VERSION;
  state.session.questions = JSON.parse(JSON.stringify(DEFAULT_STATE.session.questions));
  state.responses = {};
  state.session.round = 1;
  state.session.phase = 'closed';
  console.log('Questions changed. Saved responses cleared so old answers are not plotted on the new scales.');
  return true;
}
